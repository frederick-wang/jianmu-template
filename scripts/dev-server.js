...
async function startElectron() {
  ...
  const args = [
    Path.join(__dirname, '..', '.jianmu', 'electron', 'main.js'),
    rendererPort
  ]
...
/*
The working dir of Electron is .jianmu/electron instead of scripts/electron because of TS.
tsc does not copy static files, so copy them over manually for dev server.
*/
function copy(path) {
  FileSystem.cpSync(
    Path.join(__dirname, 'electron', path),
    Path.join(__dirname, '..', '.jianmu', 'electron', path),
    { recursive: true }
  )
}
